...
def parse_words(words):
    ...
    hyphen_climate_words = [
        c for c in CLIMATE_CHANGE_RELATED_WORDS if '-' in c]
...
def fetch_climate_words_in_words(words):
...
def fetch_climate_phrases_in_text(text):
    """Fetches climated related phrases in a string of text"""
    phrases_found = []
    for phrase in CLIMATE_CHANGE_RELATED_PHRASES:
        if phrase in text:
